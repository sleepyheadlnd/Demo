...
def divisor_game_agent(n, name, turnNum, agent):
   ...
   while len(numberList) != 0:
      ...
      if turnNum == 1:
         ...
         D = agent(numberList,-math.inf,math.inf)
         p2 = D[1]
         ...
         L = agent(numberList,-math.inf,math.inf)
         p2 = L[1]
   ...
   for x in divisors:
      ...
      while y in temp:
         if (y % temp[j] == 0):
            ...
            j += 1
      count += 1
      min_S = DG_Min_Value(temp,alpha,beta)
      temp_u = min_S[0]
      count += min_S[2]
      if temp_u > max_u:
         max_u = temp_u
   ...
   for x in divisors:
      ...
      while y in temp:
         if (y % temp[j] == 0):
            ...
            j += 1
      count += 1
      max_S = DG_Max_Value(temp,alpha,beta)
      temp_u = max_S[0]
      count += max_S[2]
      if temp_u < min_u:
         min_u = temp_u
