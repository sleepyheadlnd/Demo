...
class Agent:
   ...
   def getAction(self, percept):
      ...
      '''
     -For the cases with the higher probability of dirt and the penalty for vacuuming and moving are not
      having a hug differences, I will have the Rumba to wait 2 days if the room is clean and mov e to the next room
     -If the probability of dirt is very low and the penalty for vacuuming and moving are still not a huge different, the rumba can wait for 5 days before
      moving to a next room
      '''
      ...
      if self.p >= .1:
         ...
         if curr[STATUS] == DIRTY:
            self.numbClean = 0
            return VACUUM
         elif curr[ROOM_NUM] == 1 and self.numbClean == 3:
            self.numbClean=0
            return RIGHT
         elif curr[ROOM_NUM] == 2 and self.numbClean == 3:
            self.numbClean = 0
            return LEFT
      else:
         if curr[STATUS] == CLEAN:
         ...
         if curr[STATUS] == DIRTY:
            self.numbClean = 0
            return VACUUM
         elif curr[ROOM_NUM] == 1 and self.numbClean == 5:
            self.numbClean=0
            return RIGHT
         elif curr[ROOM_NUM] == 2 and self.numbClean == 5:
            self.numbClean = 0
            return LEFT
...
def rumba_simulation(f, agent, p, vacP, moveP, oneP, twoP):
   ...
   # COPY YOUR CODE FROM PREVIOUS PROBLEM
   status = [[], CLEAN, CLEAN]
   roomNumb = 1
...
# I changed A2P1 to A2P2 as it overrides my output file for P1
f = open('A2P2_RumbaSim_' + name + '_output.txt', 'w')
